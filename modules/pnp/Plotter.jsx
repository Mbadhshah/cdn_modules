...
export default function VectorPlotter() {
  ...
  const generateGCode = async () => {
    ...
    try {
        ...
        gcode.push(`; Pen up (lifted): ${settings.zUp}mm`);
        gcode.push(`; Pen down (on bed): ${settings.zDown}mm`);
        gcode.push(`; All Z moves use G0`);
        gcode.push(`G0 Z${settings.zUp} F${settings.travelSpeed}`); // Pen up
        ...
        paths.forEach(poly => {
            ...
            // 2. Pen down (on bed) — Z always G0
            gcode.push(`G0 Z${settings.zDown} F${settings.travelSpeed}`);
